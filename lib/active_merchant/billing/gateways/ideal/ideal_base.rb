    ...
    class IdealBaseGateway < Gateway
      ...
      def scrub(transcript)
        transcript.
          ...
          gsub(%r{(<tokenCode>)[^<]*(<\/tokenCode>)}, '\1[FILTERED]\2')
      end
      ...
      def commit(request)
        ...
        when 'AcquirerStatusRes'
          raise SecurityError, 'Message verification failed.' unless status_response_verified?(response)
        ...
        IdealResponse.new(success, response.keys[0], response, test: test?)
      end
